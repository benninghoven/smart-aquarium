...
class WaterData {
  ...
  WaterData(this.timestamp, this.temperature, this.water_temp);
}
...
Future<void> fetchData() async {
  ...
  LineChartData _buildChartData() {
    ...
    dataPoints.forEach((data) {
      spots.add(FlSpot(data.timestamp.toDouble(), data.temperature));
    });
    ...
    return LineChartData(
      ...
      minY: 0,
      maxY: dataPoints.isNotEmpty ? dataPoints.map((e) => e.temperature).reduce((a, b) => a > b ? a : b) + 10 : 10,
      lineBarsData: [
        LineChartBarData(
          spots: spots,
          isCurved: true,
          ...
          getTooltipItems: (List<LineBarSpot> touchedSpots) {
            return touchedSpots.map((LineBarSpot touchedSpot) {
              ...
              if (selectedData != null) {
                latestTemp = selectedData.water_temp; // Update latest water_temp on point hover
              }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Water Temperature °F Chart', textAlign: TextAlign.center,),
        centerTitle: true,
      ),
      body: Stack(
        children: [
